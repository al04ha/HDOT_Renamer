...
doc_registry = load_json_file(REGISTRY_FILE, {"bid_tab": {"description": "Bid tabs.", "naming_convention": "custom_bidtab_format"}})

# Pre-seeded directory fallback to protect admin elevation paths
DEFAULT_USERS = {
    "admin": {
        "password_hash": hash_password("hdotadmin2026"),
        "role": "Master Admin"
    }
}
...
# --- UPDATED PYDANTIC SCHEMA TO CAPTURE PROJECT NUMBER ---
class DocumentClassifier(BaseModel):
    identified_category: str  
    project_name: str        
    project_number: str = Field(description="The formal project number, alphanumeric code, ID, or reference sequence found in the document. If missing, return 'no-number'.")
    document_date: str        
...
# =============================================================
# VIEW 2: DEVELOPER CONTROL CONSOLE
# =============================================================
    ...
    with col_rule:
        new_convention = st.text_input("Naming Convention Rule", value="projectnumber_projectname_date")
...
# =============================================================
    ...
    # Manual text input override box
    custom_search_term = st.text_input(
        "💡 Custom Target Name Override (Optional):", 
        placeholder="Type any text here to explicitly replace 'projectname' in your filenames...",
        help="Leave this blank if you want the Gemini AI to extract the project name for you."
    )
            ...
            for uploaded_file in uploaded_files:
                with st.status(f"Parsing: {uploaded_file.name}...", expanded=False) as status:
                    try:
                        ...
                        # --- ENHANCED AI PROMPT TO SEARCH FOR PROJECT NUMBER ---
                        prompt = f"""
                        ...
                        - project_number: Locate any alphanumeric project code, tracking ID, job code, state/federal project number, or reference number. Remove spaces/slashes.
                        ...
                        # Extract the newly parsed project number
                        num = result.get("project_number", "no-number").replace(" ", "-").replace("/", "-").lower()
                        
                        # Decide whether to use manual typing override or AI project name output
                        if custom_search_term.strip():
                            proj = custom_search_term.strip().replace(" ", "-").lower()
                        else:
                            proj = result.get("project_name", "unknown-project").replace(" ", "-").lower()
                        
                        if cat not in doc_registry:
                            convention = "general_document"
                            cat_label = "UNKNOWN_GENERAL"
                        else:
                            convention = doc_registry[cat]["naming_convention"]
                            cat_label = cat.upper()
                            
                        # Intelligent Token Swap: Now tracking projectname, projectnumber, and date
                        final_name = convention
                        if "projectnumber" in final_name:
                            final_name = final_name.replace("projectnumber", num)
                        if "projectname" in final_name:
                            final_name = final_name.replace("projectname", proj)
                        if "date" in final_name:
                            final_name = final_name.replace("date", dt)
